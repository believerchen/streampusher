...
static void libenc_setEncoderPreset(JNIEnv *env, jobject thiz, jstring preset) {
    ...
    memset(x264_ctx.preset, 0x00, sizeof(x264_ctx.preset));
...
static jbyteArray libenc_BGRAToI420(JNIEnv *env, jobject thiz, jbyteArray frame, jint src_width,
                                    jint src_height, jboolean need_flip, jint rotate_degree) {
    jbyte *bgra_frame = env->GetByteArrayElements(frame, NULL);

    if (!convert_to_i420((uint8_t *) bgra_frame, src_width, src_height, need_flip, rotate_degree,
                         FOURCC_BGRA)) {
        return NULL;
    }
    ...
    env->ReleaseByteArrayElements(frame, bgra_frame, JNI_ABORT);
    return i420Frame;
}
...
static jbyteArray libenc_ARGBToI420(JNIEnv *env, jobject thiz, jbyteArray frame, jint src_width,
                                    jint src_height, jboolean need_flip, jint rotate_degree) {
    jbyte *argb_frame = env->GetByteArrayElements(frame, NULL);
...
// For Bitmap.getPixels() ARGB_8888
static jbyteArray libenc_ABGRToI420(JNIEnv *env, jobject thiz, jbyteArray frame, jint src_width,
                                    jint src_height, jboolean need_flip, jint rotate_degree) {
    jbyte *argb_frame = env->GetByteArrayElements(frame, NULL);
...
// For Bitmap.getPixels() ARGB_8888
static jbyteArray
libenc_ARGBToI420Scaled(JNIEnv *env, jobject thiz, jbyteArray frame, jint src_width,
                        jint src_height, jboolean need_flip, jint rotate_degree,
                        jint crop_x, jint crop_y, jint crop_width, jint crop_height) {
    jbyte *argb_frame = env->GetByteArrayElements(frame, NULL);
...
//// For COLOR_FormatYUV420SemiPlanar
static jbyteArray libenc_BGRAToNV12(JNIEnv *env, jobject thiz, jbyteArray frame, jint src_width,
                                    jint src_height, jboolean need_flip, jint rotate_degree) {
    jbyte *rgba_frame = env->GetByteArrayElements(frame, NULL);
...
// For Bitmap.getPixels() ARGB_8888
static jbyteArray libenc_ARGBToNV12(JNIEnv *env, jobject thiz, jbyteArray frame, jint src_width,
                                    jint src_height, jboolean need_flip, jint rotate_degree) {
    jbyte *argb_frame = env->GetByteArrayElements(frame, NULL);
...
// For Bitmap.getPixels() ARGB_8888
static jbyteArray libenc_ABGRToNV12(JNIEnv *env, jobject thiz, jbyteArray frame, jint src_width,
                                    jint src_height, jboolean need_flip, jint rotate_degree) {
    jbyte *argb_frame = env->GetByteArrayElements(frame, NULL);
    if (!argb_frame)
        return NULL;
...
// For Bitmap.getPixels() ARGB_8888
static jbyteArray
libenc_ARGBToNV12Scaled(JNIEnv *env, jobject thiz, jbyteArray frame, jint src_width,
                        jint src_height, jboolean need_flip, jint rotate_degree,
                        jint crop_x, jint crop_y, jint crop_width, jint crop_height) {
    jbyte *argb_frame = env->GetByteArrayElements(frame, NULL);
...
static int x264_encode(struct YuvFrame *i420_frame, int64_t pts) {
    ...
    LOGD("777777777  i_type = %d", pic_out.i_type);
...
static jint libenc_ABGRSoftEncode(JNIEnv *env, jobject thiz, jbyteArray frame, jint src_width,
                                  jint src_height, jboolean need_flip, jint rotate_degree,
                                  jlong pts) {
    jbyte *rgba_frame = env->GetByteArrayElements(frame, NULL);
...
static jboolean libenc_openSoftEncoder(JNIEnv *env, jobject thiz) {
    ...
    // presetting 可设置值{ "ultrafast", "superfast", "veryfast", "faster", "fast", "medium", "slow", "slower", "veryslow", "placebo", 0 };
//    可选项: film（胶片电影）, animation（动画片）, grain（颗粒感很重的）, stillimage（静态图像）, psnr（信噪比。）, ssim（）,
// fastdecode（快速解码。主要用于一些低性能的播放设备）, zerolatency（低时延。主要用于直播等）.
    //film    animation  太卡 码率太高
    x264_param_default_preset(&x264_ctx.params, "ultrafast", "zerolatency");

    //设置编码格式为YUV420I
    x264_ctx.params.i_csp = X264_CSP_I420;
...
//    x264_ctx.height = 640;
    ...
    // 瞬时最大码率
    x264_ctx.params.rc.i_vbv_max_bitrate = (x264_ctx.params.rc.i_bitrate) * 1.5;
...
//    x264_ctx.params.i_bframe = 8;
//    x264_ctx.params.b_cabac =1;
//    x264_ctx.params.i_level_idc
    //定义并行单元数目
    x264_ctx.params.i_threads = 1;
    x264_ctx.params.i_timebase_den = x264_ctx.params.i_fps_num;
    x264_ctx.params.i_timebase_num = x264_ctx.params.i_fps_den;
...
//    x264_ctx.params.i_level_idc=30;
//    x264_ctx.params.rc.f_rf_constant = 15;
//    x264_ctx.params.rc.f_rf_constant_max = 20;
...
static JNINativeMethod libenc_methods[] = {
        ...
        {"BGRAToI420",           "([BIIZI)[B",            (void *) libenc_BGRAToI420},
        {"BGRAToNV12",           "([BIIZI)[B",            (void *) libenc_BGRAToNV12},
        {"ARGBToI420Scaled",     "([BIIZIIIII)[B",        (void *) libenc_ARGBToI420Scaled},
        {"ARGBToNV12Scaled",     "([BIIZIIIII)[B",        (void *) libenc_ARGBToNV12Scaled},
        {"ARGBToI420",           "([BIIZI)[B",            (void *) libenc_ARGBToI420},
        {"ARGBToNV12",           "([BIIZI)[B",            (void *) libenc_ARGBToNV12},
        {"ABGRToI420",           "([BIIZI)[B",            (void *) libenc_ABGRToI420},
        {"ABGRToNV12",           "([BIIZI)[B",            (void *) libenc_ABGRToNV12},
        ...
        {"ABGRSoftEncode",       "([BIIZIJ)I",            (void *) libenc_ABGRSoftEncode},
};
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass clz = jenv->FindClass("com/believer/mypublisher/oort/SrsEncoder");
    if (clz == NULL) {
        LIBENC_LOGE("Class \"com/believer/mypublisher/oort/SrsEncoder\" not found");
        return JNI_ERR;
    }
