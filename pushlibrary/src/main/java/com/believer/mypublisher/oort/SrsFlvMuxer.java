...
import static com.believer.mypublisher.oort.RtmpListener.MSG_RTMP_AUDIO_STREAMING;
...
import static com.believer.mypublisher.oort.RtmpListener.MSG_RTMP_STOPPED;
import static com.believer.mypublisher.oort.RtmpListener.MSG_RTMP_VIDEO_STREAMING;
import static com.believer.mypublisher.oort.SrsEncoder.VGOP;
...
public class SrsFlvMuxer {
    ...
    private boolean connect(String url) {
        ...
        jniRtmpPointer = SrsRtmp.getInstance().open(url, true);
    ...
    private void sendFlvTag(SrsFlvFrame frame) {
        ...
        if (frame.isVideo()) {
            ...
            wtRtmp = SrsRtmp.getInstance().write(jniRtmpPointer, frame.flvTag.array(), frame.flvTag.size(), FLV_RTMP_PACKET_TYPE_VIDEO, frame.dts);
            atomicInteger.decrementAndGet();
            mVideoAllocator.release(frame.flvTag);
            rtmplistener.callRtmpState(MSG_RTMP_VIDEO_STREAMING);
        } else if (frame.isAudio()) {
            wtRtmp = SrsRtmp.getInstance().write(jniRtmpPointer, frame.flvTag.array(), frame.flvTag.size(), FLV_RTMP_PACKET_TYPE_AUDIO, frame.dts);
            mAudioAllocator.release(frame.flvTag);
            rtmplistener.callRtmpState(MSG_RTMP_AUDIO_STREAMING);
        }
        if(wtRtmp!=0){
            count++;
        }
    ...
    private void disconnect() {
        try {
            if (jniRtmpPointer != 0) {
               int close = SrsRtmp.getInstance().close(jniRtmpPointer);
            }
        } catch (IllegalStateException e) {
            // Ignore illegal state.
        }
        rtmplistener.callRtmpState(MSG_RTMP_STOPPED);
        mVideoSequenceHeader = null;
        mAudioSequenceHeader = null;
    }
...
//                    writeH264SpsPps(dts, pts);
        ...
        private void writeH264SpsPps(int dts, int pts) {
            ...
            Log.i("TTTT", String.format("flv: h264 sps/pps sent, sps=%dB, pps=%dB",
