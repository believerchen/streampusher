...
public class SrsEncoder {
    ...
    public static int vBitrate = 1200 * 1000;  // 1200 kbps
    ...
    public static final int ABITRATE = 32 * 1024;  // 32 kbps
    ...
    private MediaCodec aencoder = null;
    ...
    private boolean canSoftEncode = true;
    ...
    private boolean mbEncodecStart = false;
    ...
    private static SrsEncoder mInstance;
    private long start_times;
    private long end_times;

    public static SrsEncoder getInstance() {
        if (mInstance == null) {
            mInstance = new SrsEncoder();
        }
        return mInstance;
    }
    public boolean isEncodeStart(){
        return mbEncodecStart;
    }
    private SrsEncoder() {

    }

    public void resetEncodeParam() {
        ...
        videoFormat.setInteger("max-bitrate", (int) (vBitrate * 1.25));
...
//        videoFormat.setInteger(MediaFormat.KEY_COLOR_FORMAT, mVideoColorFormat);
        ...
        videoFormat.setInteger(MediaFormat.KEY_WIDTH, vOutWidth);
        videoFormat.setInteger(MediaFormat.KEY_HEIGHT, vOutHeight);
    ...
    public void setVideoHHDMode() {
        vBitrate = 1200 * 1000;  // 1.2Mbps
        x264Preset = "ultrafast";
    }

    public void setVideoHDMode() {
        vBitrate = 1000 * 1000;  // 1Mbps
        x264Preset = "ultrafast";
    }

    public void setVideoStanderMode() {
        vBitrate = 700 * 1000;  // 700 kbps
        x264Preset = "ultrafast";
    }
    ...
    public void setVideoVSmoothMode() {
        vBitrate = 300 * 1000;  // 300 kbps
        x264Preset = "ultrafast";
    }
    ...
    public void onGetPcmFrame(byte[] data, int size) {
        ...
        if (videoFrameCacheNumber != null && videoFrameCacheNumber.get() == 0)
            pauseAudio = false;

        if (videoFrameCacheNumber != null && videoFrameCacheNumber.get() < VGOP && !pauseAudio) {
    ...
    public void onGetAbgrFrame(byte[] data, int width, int height, long pts) {

        //如果停止了推流马上返回
        if(!mbEncodecStart)
            return;
        //判断帧率是否在控制范围内，比如24帧，帧间隔应该控制在40毫秒，帧率过大导致编码压力
        AtomicInteger videoFrameCacheNumber = flvMuxer.getVideoFrameCacheNumber();
        //如果缓冲过多先清理缓冲, 用于测试卡顿
        if (videoFrameCacheNumber != null && videoFrameCacheNumber.get() >= VGOP) {
            pauseVideo = true;
            ...
            Log.i("卡顿了，泥煤", "卡顿时间 ：" + str);
        }

        if (videoFrameCacheNumber != null && videoFrameCacheNumber.get() == 0)
            pauseVideo = false;

        if (videoFrameCacheNumber != null && videoFrameCacheNumber.get() < VGOP && !pauseVideo) {
            long curPts = System.nanoTime() / 1000 - mPresentTimeUs;
            //long curPts = pts;
            if (useSoftEncoder) {
                //swRgbaFrame(data, width, height, pts);
                swAbgrFrame(data, width, height, curPts);
                end_times = System.currentTimeMillis();
                Log.i("===========>使用软编码", "66666666666666666666666666  PTS：" + curPts);
            } else {
                byte[] processedData = hwAbgrFrame(data, width, height);
                end_times = System.currentTimeMillis();
                Log.i("===========>使用硬编码", "6666666666666666666666666611  PTS：" + curPts);
                if (processedData != null) {
                    //数据返回显示
//                ByteArrayOutputStream out = new ByteArrayOutputStream();
//                YuvImage yuvImage;
//                yuvImage = new YuvImage(processedData, ImageFormat.NV21, width, height, null);
//                yuvImage.compressToJpeg(new Rect(0, 0, width, height), 100, out);
//                byte[] imageBytes = out.toByteArray();
//                Bitmap image = BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.length);
//                EventBus.getDefault().post(new Bitmap_Event(10086,image));

                    onProcessedYuvFrame(processedData, curPts);
                } else {
    ...
    private byte[] hwAbgrFrame(byte[] data, int inputWidth, int inputHeight) {
        switch (mVideoColorFormat) {
            case MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Planar:
                return ABGRToI420(data, inputWidth, inputHeight, false, 0);
            case MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar:
                return ABGRToNV12(data, inputWidth, inputHeight, false, 0);
            default:
                throw new IllegalStateException("Unsupported color format!");
        }
    }

    private void swAbgrFrame(byte[] data, int width, int height, long pts) {
        ABGRSoftEncode(data, width, height, false, 0, pts);
    }
    ...
    private native byte[] BGRAToI420(byte[] frame, int width, int height, boolean flip, int rotate);

    private native byte[] BGRAToNV12(byte[] frame, int width, int height, boolean flip, int rotate);

    private native byte[] ARGBToI420Scaled(byte[] frame, int width, int height, boolean flip, int rotate, int crop_x, int crop_y, int crop_width, int crop_height);

    private native byte[] ARGBToNV12Scaled(byte[] frame, int width, int height, boolean flip, int rotate, int crop_x, int crop_y, int crop_width, int crop_height);

    private native byte[] ARGBToI420(byte[] frame, int width, int height, boolean flip, int rotate);

    private native byte[] ARGBToNV12(byte[] frame, int width, int height, boolean flip, int rotate);

    private native byte[] ABGRToI420(byte[] frame, int width, int height, boolean flip, int rotate);

    private native byte[] ABGRToNV12(byte[] frame, int width, int height, boolean flip, int rotate);
    ...
    private native int ABGRSoftEncode(byte[] frame, int width, int height, boolean flip, int rotate, long pts);
